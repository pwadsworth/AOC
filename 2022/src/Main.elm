...
-- UPDATE the DayX module import to test against your solution
...
title =
    el [ Font.size 30, Font.italic, Font.semiBold, centerX, padding 10 ]
        (text "Advent of Code 2022 with Elm")
...
mainArea model =
    let
        ( part1, part2 ) =
            Day.solution model.input
    in
    row [ width fill ]
        [ column [ alignTop ]
            [ row [ padding 5 ] [ text <| "Result Part 1: " ++ part1 ]
            , row [ padding 5 ] [ text <| "Result Part 2: " ++ part2 ]
            ]
        , inputsArea model
        ]
